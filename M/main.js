...
textureLoader.load('backm.JPG', (texture) => {
    // 背景テクスチャの設定
...
// アニメーションミキサーの設定
const mixers = [];
let currentAction1 = null; // 現在のアニメーションアクションを追跡
...
function triggerNodAnimation() {
    if (currentAction1 && animationTriggerCount < maxTriggers) {
        ...
        currentAction1.play(); // アニメーションをリセットして再生
...
// 5秒ごとにアニメーションをトリガーする
let animationInterval = setInterval(triggerNodAnimation, 5000); // 5秒ごとに呼び出し
